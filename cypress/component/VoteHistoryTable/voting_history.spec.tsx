...
describe('VotingHistory Component', () => {
  it('Renders VotingHistory component with correct headers', () => {
    ...
    cy.contains('div', 'Votes')
    cy.contains('div', 'Chair Votes')
    cy.contains('div', 'Member Votes')
  })
  it('Renders VotingHistory component with correct elections and votes', () => {
    cy.mount(<VotingHistory darCollection={darCollection} dacIds={dacIds} />)
    cy.get('.table-data').eq(0).as('chairTable')
    cy.get('.table-data').eq(1).as('memberTable')
    ...
    cy.get('@chairTable').contains('2022-11-21')
    cy.get('@chairTable').should('not.contain', '2023-01-03')
    cy.get('@memberTable').contains('2022-11-21')
    cy.get('@memberTable').should('not.contain', '2023-01-03')
    ...
    cy.get('@chairTable').contains('2022-11-21')
    cy.get('@chairTable').should('not.contain', '2023-01-10')
    cy.get('@memberTable').contains('2022-11-21')
    cy.get('@memberTable').should('not.contain', '2023-01-10')
    // Chair table only includes final or radar approve votes
    cy.get('@chairTable').contains(VOTE_TYPES.FINAL)
    cy.get('@chairTable').contains(VOTE_TYPES.RADAR_APPROVE)
    cy.get('@chairTable').should('not.contain', 'DAC')
    cy.get('@chairTable').should('not.contain', 'AGREEMENT')
    cy.get('@chairTable').should('not.contain', 'Chairperson')
    // Chair table only includes populated votes
    cy.get('@chairTable').contains('Stuart Williams')
    cy.get('@chairTable').should('not.contain', 'Ted Lasso')
    // votes in member table should be filtered by DAC type and contains populated and unpopulated votes
    cy.get('@memberTable').find('[data-testid="ExpandMoreIcon"]').first().click()
    cy.get('@memberTable').contains('Beth Johnson')
    cy.get('@memberTable').contains('DAC Member 1')
    cy.get('@memberTable').contains('DAC Member 2')
    cy.get('@memberTable').should('not.contain', 'Ted Lasso')
    cy.get('@memberTable').should('not.contain', 'Stuart Williams')
    cy.get('@memberTable').should('not.contain', 'Sue Smith')
  })
})
