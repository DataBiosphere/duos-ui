...
describe('Summary Section - Component Tests', () => {
  ...
  it('shows era authenticated researcher identification', () => {
    const authedResearcher = {
    ...
    mountComponent({}, {
      eRACommonsDestination: 'progress_report_application/collection-id',
      location,
      researcher: authedResearcher
    });
    cy.get('[data-cy=researcher-identification]').should('exist');
    cy.get('[data-cy=researcher-identification]').should(($p) => {
      expect($p).to.contain(researcher.eraCommonsId);
    })
  });
