...
describe('BucketUtils', () => {
  ...
  describe('algorithm version handling', () => {
    ...
    // Secondary modifiers are not primary, so V5 matches them normally.
    ...
    it.each(['v1', 'v2'])('reports a %s result as uninterpretable rather than suppressing it', async (version) => {
      const matchable = await bucketFor({ primary: [GRU] }, [makeMatch(version, { match: true })])
      const unmatchable = await bucketFor({ primary: [OTHER] }, [makeMatch(version, { match: true })])

      expect(matchable.algorithmResult?.result).toBe('System match unavailable for this algorithm version')
      expect(matchable.algorithmResult?.rationales?.[0]).toContain(version)
      expect(unmatchable.algorithmResult?.result).toBe('System match unavailable for this algorithm version')
    })
    ...
    it('reads a missing version as uninterpretable', async () => {
    ...
    it('reports versions DUOS reads differently rather than interpreting the newest', async () => {
      ...
      expect(bucket.algorithmResult?.rationales?.[0]).toContain('v2')
      expect(bucket.algorithmResult?.rationales?.[0]).not.toContain('v5')
    })
