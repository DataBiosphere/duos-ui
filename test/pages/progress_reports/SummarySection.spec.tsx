...
describe('SummarySection', () => {
  ...
  it('renders the component correctly', () => {
    renderComponent()
    expect(document.querySelector('[data-cy="summary-section"]')).toBeInTheDocument()
    expect(screen.getByText('Step 1: Submit a Progress Report')).toBeInTheDocument()
    expect(screen.getByText('1.1 Researcher Identification')).toBeInTheDocument()
    expect(screen.getByText('1.2 Summary of Progress')).toBeInTheDocument()
    expect(screen.getByText('1.3 Intellectual Property')).toBeInTheDocument()
    expect(screen.getByText('1.4 Publications')).toBeInTheDocument()
    expect(screen.getByText('1.5 Presentations')).toBeInTheDocument()
  })

  it('renders in read-only mode correctly', () => {
    renderComponent({}, true)
    expect(screen.getByText('Review a Progress Report')).toBeInTheDocument()
  ...
  it('displays the correct descriptions for each section', () => {
    ...
    expect(screen.getByText(/Please summarize your research on this project/)).toBeInTheDocument()
    // 1.3 Intellectual Property — text node before the <strong> is matched; the bold word is checked separately
    expect(screen.getByText(/Have you generated any/)).toBeInTheDocument()
    expect(screen.getByText('intellectual property (IP)')).toBeInTheDocument()
  ...
  it('handles intellectual property radio buttons', () => {
    ...
    expect(onFormChange).toHaveBeenCalledWith(expect.objectContaining({ intellectualPropertiesYesNo: true }))
  })

  it('shows intellectualProperties list when "Yes" is selected', () => {
    renderComponent({ intellectualPropertiesYesNo: true })
    expect(screen.getByRole('button', { name: 'Add IP' })).toBeInTheDocument()
  })
  ...
  it('handles publications radio buttons', () => {
    ...
    fireEvent.click(yesRadio)
    expect(onFormChange).toHaveBeenCalledWith(expect.objectContaining({ publicationsYesNo: true }))
  })

  it('shows publications list when "Yes" is selected', () => {
    renderComponent({ publicationsYesNo: true })
    expect(screen.getByTestId('publication-list')).toBeInTheDocument()
  })
  ...
  it('handles presentations radio buttons', () => {
    ...
    fireEvent.click(yesRadio)
    expect(onFormChange).toHaveBeenCalledWith(expect.objectContaining({ presentationsYesNo: true }))
  })
  ...
  it('shows era authenticated researcher commons id', () => {
    ...
    expect(document.querySelector('[data-cy="era-commons-display-id-value"]')).toBeInTheDocument()
  })

  it('shows authentication message for un-authed user', () => {
    renderComponent()
    expect(screen.getByText('Authenticate your account')).toBeInTheDocument()
