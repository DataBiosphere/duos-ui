...
const createVoteRecord = (dar: DataAccessRequestModel, datasetId: number, election: Election | undefined, datasets: Dataset[]): VoteRecord => {
  ...
  const resolveVoteDate = () => {
    if (hasFinalVote) return finalVote?.updateDate ?? finalVote?.createDate ?? null
    if (isElectionClosed) return election?.createDate ?? null
    return null
  }

  const voteDateRaw = resolveVoteDate()
  const undatedLabel = election ? NO_FINAL_VOTE_STATUS : NO_ELECTION_STATUS
  ...
  return {
    datasetId,
    datasetName,
    voteDate: voteDateRaw === null ? undatedLabel : formatDate(voteDateRaw),
    voteDateRaw,
