...
import {DAC, DAR, DataSet, Files, User} from '../libs/ajax';
...
export default function DACDatasets(props) {
  ...
  const getDatasets = useCallback(async () => {
    // todo: replace api call with api/user/me/dac/datasets
    // this gets both chair/member DAC datasets
    let datasets = await User.getUserRelevantDatasets();
    // filter to chairperson only
        ...
        div({ className: 'row no-margin' }, [
          div({ className: 'col-lg-7 col-md-7 col-sm-12 col-xs-12 no-padding' }, [
            PageHeading({
              ...
              color: 'dataset',
              title: 'DAC Datasets',
              description: 'Datasets managed by your Data Access Committee'
            }),
          ]),
          div({ className: 'right'}, [
            ...
            button({
              ...
              onClick: () => props.history.push({ pathname: 'data_submission_form' }),
