          ...
          <tbody key="dac_datasets_table_body">
            {
              datasets.map((dataset, index) => {
                return (
                  <tr key={'dac_datasets_table_row_' + index + '_' + dataset.datasetIdentifier}>
                    <td
                      key={'1_' + dataset.datasetIdentifier}
                    ...
                    <td
                      key={'3_' + dataset.datasetIdentifier}
                      className="table-items cell-size"
                    >
                      {getDbGapLinkValue(dataset.properties)}
                    </td>
                    <td
                      key={'4_' + dataset.datasetIdentifier}
                      className="table-items cell-size"
                    >
                      {getStructuredUseRestrictionLink(index)}
                    </td>
                    <td
                      key={'5_' + dataset.datasetIdentifier}
                      className="table-items cell-size"
                    >
                      {getPropertyValue(dataset.properties, 'Data Type', '---')}
                    </td>
                    <td
                      key={'6_' + dataset.datasetIdentifier}
                      className="table-items cell-size"
                    >
                      {getPropertyValue(dataset.properties, 'Phenotype/Indication', '---')}
                    </td>
                    <td
                      key={'7_' + dataset.datasetIdentifier}
                      className="table-items cell-size"
                    >
                      {dataset?.study?.piName || getPropertyValue(dataset.properties, 'Principal Investigator(PI)', '---')}
                    </td>
                    <td
                      key={'8_' + dataset.datasetIdentifier}
                      className="table-items cell-size"
                    >
                      {getPropertyValue(dataset.properties, '# of participants', '---')}
                    </td>
                    <td
                      key={'9_' + dataset.datasetIdentifier}
                      className="table-items cell-size"
                    >
                      {getPropertyValue(dataset.properties, 'Description', '---')}
                    </td>
                    <td
                      key={'10_' + dataset.datasetIdentifier}
                      className="table-items cell-size"
                    >
                      {getPropertyValue(dataset.properties, 'Species', '---')}
                    </td>
                    <td
                      key={'11_' + dataset.datasetIdentifier}
                      className="table-items cell-size"
                    >
                      {getPropertyValue(dataset.properties, 'Data Depositor', '---')}
                    </td>
                  </tr>
                )
              })
