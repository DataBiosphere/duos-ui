...
export default function ERACommons(props) {
  ...
  return (
    <div id="era-commons-id" style={{ minHeight: 65 }}>
      ...
      {nihError && (
        <span
          data-cy="era-commons-error-span"
          className="era-cancel-color era-required-field-error-span"
        >
          {nihError}
        </span>
      )}
      {isAuthorized && (
        <div>
          ...
          <div className="era-expiration-value">
            {expirationCount >= 0 && (
              <div
                className="era-fadein"
              >
                {`Your NIH authentication will expire in ${expirationCount} days`}
              </div>
            )}
            {expirationCount < 0
