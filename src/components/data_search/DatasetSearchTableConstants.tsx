...
export const makeStudyTableHeaders = (datasets: DatasetTerm[], selected: number[], onSelect: (datasetIds: number[]) => void, _exportableDatasets: { [duosId: string]: SnapshotSummaryModel[] }): HeaderData<DatasetTerm[]>[] => {
  ...
  return [
    ...
    },
    {
      ...
      cellDataFn: datasets => ({
        data: (
          <OverflowTooltip tooltipText={datasets[0].study.phenotype} id={`${datasets[0].study.studyId}-study-phenotype-data`}>
    ...
    },
    {
      ...
      cellDataFn: datasets => ({
        data: (
          <OverflowTooltip tooltipText={datasets[0].study.species} id={`${datasets[0].study.studyId}-study-species-data`}>
    ...
    },
    {
      ...
      cellDataFn: datasets => ({
        data: (
          <OverflowTooltip tooltipText={datasets[0].study.piName} id={`${datasets[0].study.piName}-study-pi-name-data`}>
    ...
    },
    {
      ...
      cellDataFn: (datasets) => {
        ...
        return {
          data: (
            <OverflowTooltip tooltipText={custodians} id={`${datasets[0].study.studyId}-study-custodian-data`}>
              {custodians}
            </OverflowTooltip>
          ),
          value: custodians,
...
export const makeDatasetTableHeader = (
  ...
  return [
    ...
    },
    {
      ...
      cellDataFn: (dataset: DatasetTerm) => ({
        data: (
          <OverflowTooltip tooltipText={dataset.datasetName} id={`${dataset.datasetId}-dataset-name`}>
            <Link
    ...
    },
    {
      ...
      cellDataFn: (dataset: DatasetTerm) => ({
        data: (
          <OverflowTooltip
            tooltipText={dataset.study.studyName}
    ...
    },
    {
      ...
      cellDataFn: (dataset: DatasetTerm) => ({
        data: (
          <OverflowTooltip
            tooltipText={dataset.study.dataTypes?.join(', ')}
    ...
    },
    {
      ...
      cellDataFn: (dataset: DatasetTerm) => {
        ...
        const safeDatasetUrl = getSafeHttpUrl(dataset.url)
        if (dataset.dataLocation === DataLocationType.TDRLocation) {
          ...
          dataLocation = safeDatasetUrl
            ? <Link href={safeDatasetUrl}>{DataLocationType.TerraWorkspace}</Link>
            : TERRA_DATA_REPO
        }
          ...
          dataLocation = safeDatasetUrl ? new URL(safeDatasetUrl).hostname : ''
        }
        else {
          dataLocation = safeDatasetUrl
            ? <Link href={safeDatasetUrl}>External to DUOS</Link>
            : 'External Location'
        }
        return {
          data: dataLocation,
