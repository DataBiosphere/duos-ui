...
const recordAutoLogout401 = async (url: string): Promise<void> => {
  ...
  try {
    ...
    // Deliberately not awaited: this handler runs inside the rejected 401's
    // path, and redirectOnLogout consumes its own result (including the
    // unconfirmed-sign-out notice) and never rejects.
