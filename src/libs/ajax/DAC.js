...
export const DAC = {
  list: async (withUsers) => {
    const url = `${await getApiUrl()}/api/dac` + (isEmpty(withUsers) ? '' : `?withUsers=${withUsers}`);
  ...
  create: async (name, description, email) => {
    ...
    const res = await fetchOk(url, mergeAll([Config.authOpts(), Config.jsonBody(dac), { method: 'POST' }]));
    return res.json();
  },
  ...
  update: async (dacId, name, description, email) => {
    ...
    const res = await fetchOk(url, mergeAll([Config.authOpts(), Config.jsonBody(dac), { method: 'PUT' }]));
    return res.json();
  },
  ...
  updateApprovalStatus: async (dacId, datasetId, approvalStatus) => {
    ...
    const res = await fetchOk(url, mergeAll([Config.authOpts(), Config.jsonBody(approval), { method: 'PUT' }]));
    return res.json();
  },
