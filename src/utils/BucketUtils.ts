...
// The dataset_id migration restamped the rows that predated the version column, so an absent
// version is a row DUOS can no longer account for rather than a legacy one.
const isInterpretableVersion = (version: string | undefined): boolean => {
...
const calculateAlgorithmResultForBucket = (bucket: Bucket): AlgorithmResult => {
  ...
  const versions: (string | undefined)[] = chain(bucket.matchResults)
    ...
    .uniq()
    .value()
  const unreadableVersions = versions.filter(v => !isInterpretableVersion(v))
  if (!isEmpty(unreadableVersions)) {
    // No single createDate applies when the rows come from runs DUOS reads differently.
    return {
      result: UNRECOGNIZED_ALGORITHM_RESULT,
      createDate: undefined,
      rationales: [
        `This match includes results from algorithm version ${unreadableVersions.map(v => v ?? 'unknown').join(', ')}, `
