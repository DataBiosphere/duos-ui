...
describe('connectSources', () => {
  ...
  it.each([
    ...
    expect(connectSources({ bffEnabled: true, apiUrl, bannersUrl: BANNERS_URL }, PROD)).toEqual(['\'self\'', BANNER_SOURCE])
  })
  ...
  it('emits no banner source when config.json has no bannersUrl', () => {
    ...
    expect(connectSources(config, PROD)).toEqual(['\'self\'', CONSENT, BARD])
  })
