...
Add a validation workflow for completed study/dataset CSV templates. A data submitter uploads a template, validates it without submitting a study, corrects any reported errors in the source file, and uploads a replacement. A valid template creates a DUOS draft and routes the user to a populated draft study registration form for review.
...
Consent already provides draft persistence under `/api/draft/v1`. `DraftStudyDataset` stores a
registration-shaped JSON document under a UUID without validating its schema, so the template
workflow should extend that persistence mechanism instead of introducing another draft model.
...
- Creation of a `DraftStudyDataset` only after successful validation.
...
sequenceDiagram
    ...
    alt Template contains validation errors
        ...
        API->>DB: Create DraftStudyDataset
        DB-->>API: draft UUID
        API-->>UI: 200 { valid: true, draftId: UUID }
        UI->>UI: Navigate to /data_submission_form/draft/:draftId
        UI->>API: GET /api/draft/v1/:draftId
        API-->>UI: Draft document and metadata
...
```text
POST /api/draft/v1/template-validation
Content-Type: multipart/form-data
file: <CSV>
```

The endpoint should allow the same roles as existing study drafts: `ADMIN` and `DATASUBMITTER`.
`CHAIRPERSON` can continue to use manual registration but is not included in the template workflow
because the existing draft read and delete endpoints do not authorize that role. The duos-ui action
and route must enforce the same scope.
...
{
  "valid": true,
  "draftId": "c2e4583a-20b9-4705-8280-e6a5753f10c9",
  "errors": []
}
```
...
1. Extend `DraftResource` with the multipart validation endpoint.
2. Add a template-validation service responsible for file checks, parsing, DTO mapping, and validation orchestration.
...
6. Add the endpoint and schemas to `src/main/resources/assets/api-docs.yaml`.
...
for data submitters and administrators. Preserve **ADD DATASET** as the entry point for manual
registration. Chairpersons remain on the existing manual workflow.
...
```text
/data_submission_form/draft/:draftId
```
...
- Enforce `ADMIN` and `DATASUBMITTER` roles and reject a chairperson without either role.
...
- Successful validation navigates to the returned draft UUID.
...
| Route to a populated draft on success | Persist a valid `DraftStudyDataset`, navigate to the explicit draft route, and hydrate the form from its document. |
...
| Authorized UI roles | A. Match study creation and include chairpersons.<br>B. Match existing draft APIs: data submitters and administrators.<br>C. Data submitters only. | **B.** This avoids widening all draft read/delete permissions as part of a CSV workflow. Chairpersons retain manual registration. |
...
### Ticket 3: Expose template validation and create valid study drafts
...
Add the authenticated multipart template-validation endpoint and create a study draft only when
validation succeeds.

**Description**

Extend `DraftResource` with `POST /api/draft/v1/template-validation`. Accept one CSV file, invoke
the Ticket 2 service, and return the discriminated response documented in this plan. For valid
templates, serialize the validated `StudyRegistrationRequest` as the registration-shaped document
inside a `DraftStudyDataset` and persist it through `DraftService`. Add a contract test proving the
serialized document can be read by duos-ui and posted to the current registration endpoint after
user edits. Invalid templates return structured errors and perform no database writes.
...
- `ADMIN` and `DATASUBMITTER` callers can validate and read a template draft.
- A caller with only `CHAIRPERSON` is rejected and continues to use manual registration.
...
- Successful validation returns HTTP 200 with `valid: true`, an empty error list, and a draft UUID.
- Exactly one `DraftStudyDataset` owned by the caller is created for a valid request.
...
- Resource tests for multipart, roles, status codes, and response shapes.
...
Allow data submitters to upload, validate, remove, and replace a study CSV template with immediate
feedback.
...
- Data submitters and administrators can access the action and route; a chairperson without either
  role cannot and continues to use manual registration.
...
- A valid response routes to the exact draft UUID returned by Consent.
...
Add the explicit `/data_submission_form/draft/:draftId` route and a typed client for the existing
draft read and delete endpoints. Extend `DataSubmissionFormV2` with an explicit draft mode instead
of treating a UUID as a persisted study ID. Map the registration-shaped draft document into the UI
`Study` model, including scalar fields, study properties, consent groups, and supported non-file
assets. Keep institutional-certification and alternative-sharing-plan upload controls empty and
...
- Form tests for representative scalar, property, consent-group, non-file asset, file-control, and
  `data` values.
- Route-mode and authorization-error tests.
...
- A data submitter cannot load another user's template draft, and a chairperson without a data
  submitter or administrator role cannot use the workflow.
